<?php
// Define theme version (used as cache buster)
define('WEBCO_THEME_VERSION', '1.1');
...
include 'meta-box-def.php';


// Add function to find the url of a file in the current theme
if(!function_exists('theme_url')) {
	function theme_url($url) {
		return rtrim(get_template_directory_uri(), '/') . '/' . ltrim($url, '/');
	}
}

// Include the scripts needed for this theme
function webco_scripts() {
	wp_enqueue_script('jquery');
}
add_action('wp_enqueue_scripts', 'webco_scripts');
...
function create_post_type_nieuws() {
	register_post_type( 'nieuws',
		...
		'has_archive' => true,
		'show_ui' => true,
		'supports' => array('title','page-attributes','author','custom-fields','editor','thumbnail','excerpt','templates'),
